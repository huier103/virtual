...
static int do_read_resp(struct xen_chrif *xen_chrif, struct chrif_request *req, struct chrif_response *resp)
{
	...
	err = chrbk->chrif_filp->f_op->read(chrbk->chrif_filp, chrif->block_buf, chrif->pdma_info.wt_block_sz, &chrbk->chrif_filp->f_pos);	
	...
	for(i = 0; i < count; i++){
	    ...
	    //for(k = 0; k < 5; k++)
           // printk("\n xen:dom0: read->data[%d]: 0x%x", k, ((unsigned char *)map_page)[k]);
    }
	memset(chrif->block_buf, 0, chrif->pdma_info.wt_block_sz);
...
//  int k;
	...
	for(i = 0; i < count; i++){
        map_page = chrif->map_pages[i];
		//printk(KERN_DEBUG "\nxen: dom0: the %d page addr: 0x%x ",i, map_page);
...
*/
    ...
    if(chrbk->writed_sz >= req->length){ 
		chrbk->writed_sz = 0;       
		//kfree(chrif->block_buf);  //need cancel when have read request
