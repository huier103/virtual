...
static int  block_info_to_front(struct xen_chrif *chrif)
{
    int err;
    struct file *filp;
	...
	old_fss = get_fs();
	set_fs(get_ds());
	err =  filp->f_op->unlocked_ioctl(filp, PDMA_IOC_INFO, (unsigned long)&chrif->pdma_info);	
	set_fs(old_fss);
...
/* don't have pdma, in order to test others, so note this
   if(pdma_info_flags == 0){
		pdma_info_flags = -1;
        err = block_info_to_front(be->chrif);
    }
...
*/
	...
	be->chrif->map_pages = kmalloc(sizeof(struct vm_struct) * nr_page, GFP_KERNEL);
    return 0;
