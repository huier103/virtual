...
static int chrif_map(struct xen_chrif *chrif, unsigned long ring_ref, unsigned int evtchn)
{
    ...
    err = bind_interdomain_evtchn_to_irqhandler(chrif->domid, evtchn, chrif_interrupt, 0, "domtest2", chrif);
    if (err < 0) {
        printk(KERN_DEBUG "\nxen: dom0: chrif_interrupt failed binding to evtchn");
...
static int xen_connect_ring(struct backend_info *be)
{
...
static void xen_connect(struct backend_info *be)
{
    int err;
	struct xenbus_device *dev = be->dev;

	err = xen_connect_ring(be);
	if(err){
...
static struct xen_chrif* xen_chrif_alloc(domid_t domid)
{
	...
	chrif->domid = domid;
    init_completion(&chrif->comp_waitser);
   ...
   **/
   /* don't have pdma, in order to test others, so note this */
    if(pdma_info_flags == 0){
        ...
        printk(KERN_ALERT"\nxen: dom0: only the first time get block info");
  	}
//      be->chrif->pdma_info.wt_block_sz = 16384;
    ...
    if(err){
	    ...
	    printk(KERN_DEBUG "\nxen: Dom0: wt_block_sz:  0x%lx", pdma_info.wt_block_sz);
	    xenbus_transaction_start(&xbt);
	    xenbus_printf(xbt, dev->nodename, "share_memory_pn", "%u", SHARE_MEMORY_PN);
...
*/      xenbus_transaction_end(xbt, 0);
	...
	printk("\nxen: dom0: alloc map_pages nr_page: %d", SHARE_MEMORY_PN);
	be->chrif->map_pages = kmalloc(sizeof(unsigned long) * SHARE_MEMORY_PN, GFP_KERNEL);
    memset(be->chrif->map_pages, 0, SHARE_MEMORY_PN * sizeof(unsigned long));
    return 0;
