	...
	printk(KERN_INFO "\n------------------------------start response-------------------------------------");
		...
		printk(KERN_DEBUG "\nxen: Dom0: operation:  %s", op_name(resp.operation));
      		switch(resp.operation) {
				...
				old_fs = get_fs();
				set_fs(get_ds());
				err =info.chrif_filp->f_op->read(info.chrif_filp, op_data, resp.rdwr.len, &info.chrif_filp->f_pos);	
				set_fs(old_fs);
                                if(err < 0)
					printk(KERN_DEBUG "\nxen: Dom0: read %u bytes error", resp.rdwr.len);
			         //put proc_data into page 
				memset(op_page->addr, 0, resp.rdwr.len);
				memcpy(op_page->addr, op_data, resp.rdwr.len);
				printk(KERN_DEBUG "\nxen: dom0: get the  read  data: %s", op_data);
			...
			case CHRIF_OP_IOCTL:{
				...
				switch(resp.ioc_parm.cmd){
					...
					case PDMA_IOC_INFO:{
						struct pdma_info pdma_info={ .rd_pool_sz = 0, };
						//err = call_ioctl(info.chrif_filp, PDMA_IOC_INFO, (unsigned long)&pdma_info);	
						old_fs = get_fs();
						set_fs(get_ds());
						err = info.chrif_filp->f_op->unlocked_ioctl(info.chrif_filp, PDMA_IOC_INFO, &pdma_info);	
						set_fs(old_fs);
					 	if(err){
					...
					case PDMA_IOC_STAT:{
						struct pdma_stat pdma_stat={ .rd_free_cnt = 0, };
						//err = call_ioctl(info.chrif_filp, PDMA_IOC_STAT, (unsigned long)&pdma_stat);	
						old_fs = get_fs();
						set_fs(get_ds());
						err = info.chrif_filp->f_op->unlocked_ioctl(info.chrif_filp, PDMA_IOC_STAT, &pdma_stat);	
						set_fs(old_fs);
					 	if(err){
					...
					case PDMA_IOC_RW_REG:{
						...
						old_fs = get_fs();
						set_fs(get_ds());
						err = info.chrif_filp->f_op->unlocked_ioctl(info.chrif_filp, PDMA_IOC_RW_REG, &ctrl);	
						set_fs(old_fs);
					 	if(err){
