...
static int get_block_info(){
	...
	if(err)
        printk(KERN_INFO "\nxen: dom0: get block info ioctl info fail");
    set_fs(old_fs);
    len = pdma_info.rd_block_sz;
    block_buf = (char *)kmalloc(len, GFP_KERNEL);
    printk(KERN_INFO "\nxen: dom0: block size is %d", pdma_info.rd_block_sz);
    return 0;
}
