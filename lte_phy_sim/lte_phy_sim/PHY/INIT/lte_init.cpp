...
int phy_init_lte_ue(PHY_VARS_UE *phy_vars_ue,
  ...
  LTE_UE_PRACH **ue_prach_vars        = phy_vars_ue->lte_ue_prach_vars;
   PRACH_RESOURCES_t **prach_resource_new =phy_vars_ue->prach_resources;
  int i,j;
  ...
  for(eNB_id = 0; eNB_id < NUMBER_OF_eNB_MAX; eNB_id++)
	  prach_resource_new[eNB_id]=(PRACH_RESOURCES_t *)malloc16(sizeof(PRACH_RESOURCES_t));//..wanlin此处的问题，主要由于rnti引起。
  for(eNB_id = 0; eNB_id < NUMBER_OF_eNB_MAX; eNB_id++){
		     ...
		     unsigned char abstraction_flag)
{
  ...
  for (UE_id=0; UE_id<NUMBER_OF_UE_MAX; UE_id++) {
	...
	eNB_pusch_vars[UE_id]->rxdataF_comp[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	...
	if(cooperation_flag == 2)
	  {
	    ...
	    eNB_pusch_vars[UE_id]->rxdataF_comp_0[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	    ...
	    eNB_pusch_vars[UE_id]->rxdataF_comp_1[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	...
	eNB_pusch_vars[UE_id]->ul_ch_mag[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	...
	eNB_pusch_vars[UE_id]->ul_ch_magb[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	...
	if(cooperation_flag == 2) // for Distributed Alamouti Scheme
	  {
	    // UE 0
	    eNB_pusch_vars[UE_id]->ul_ch_mag_0[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	    ...
	    eNB_pusch_vars[UE_id]->ul_ch_magb_0[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	    ...
	    // UE 1
	    eNB_pusch_vars[UE_id]->ul_ch_mag_1[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
	    ...
	    eNB_pusch_vars[UE_id]->ul_ch_magb_1[eNB_id] = (s32 **)malloc16(frame_parms->nb_antennas_rx*sizeof(int**));
