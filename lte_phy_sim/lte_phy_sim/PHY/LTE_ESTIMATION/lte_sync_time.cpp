...
__declspec(align(16))int sync_tmp[2048*4];
__declspec(align(16))short syncF_tmp[2048*2];
...
void lte_sync_time_free(void) {
  ...
  if (sync_corr_ue0) {
    ...
    _aligned_free(sync_corr_ue0);
  }
  if (sync_corr_ue1) {
    msg("Freeing sync_corr_ue (%p)...\n",sync_corr_ue1);
    _aligned_free(sync_corr_ue1);
  }
  if (sync_corr_ue2) {
    msg("Freeing sync_corr_ue (%p)...\n",sync_corr_ue2);
    _aligned_free(sync_corr_ue2);
  }
  if (primary_synch0_time) {
    msg("Freeing primary_sync0_time ...\n");
    _aligned_free(primary_synch0_time);
  }
  if (primary_synch1_time) {
    msg("Freeing primary_sync1_time ...\n");
    _aligned_free(primary_synch1_time);
  }
  if (primary_synch2_time) {
    msg("Freeing primary_sync2_time ...\n");
    _aligned_free(primary_synch2_time);
  }
#endif
  sync_corr_ue0 = NULL;
  sync_corr_ue1 = NULL;
