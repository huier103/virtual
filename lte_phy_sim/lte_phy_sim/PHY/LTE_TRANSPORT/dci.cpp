...
#ifndef __SSE3__
__declspec(align(16))__m128i zero2;
...
void dci_decoding_procedure0(LTE_UE_PDCCH **lte_ue_pdcch_vars,u8 subframe,
  ...
  for (CCEind=0;
	...
	if (crc==si_rnti) {
	  dci_alloc[*dci_cnt].format     = (DCI_format_t)format_si;
	  *dci_cnt = *dci_cnt+1;
	}
	else if (crc==ra_rnti) {
	  dci_alloc[*dci_cnt].format     = (DCI_format_t)format_ra;
	  ...
	  *dci_cnt = *dci_cnt+1;
	  ...
	  if ((format_c == format0)&&((dci_decoded_output[0]&0x80)==0)) {// check if pdu is format 0 or 1A
	    ...
	    *dci_cnt = *dci_cnt+1;
	    ...
	    // store first nCCE of group for PUCCH transmission of ACK/NAK
	    if (*format_c_found == 0) {
	      dci_alloc[*dci_cnt].format     = (DCI_format_t)format_c;
	      *dci_cnt = *dci_cnt+1;
	      *format_c_found = 1;
	      lte_ue_pdcch_vars[eNB_id]->nCCE[subframe]=CCEind;
	    }
