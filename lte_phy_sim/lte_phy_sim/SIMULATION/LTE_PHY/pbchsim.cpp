...
void lte_param_init(unsigned char N_tx, unsigned char N_rx,unsigned char transmission_mode,unsigned char extended_prefix_flag,unsigned char frame_type, u16 Nid_cell,u8 N_RB_DL,u8 osf) {
  ...
  PHY_vars_eNb1 = (PHY_VARS_eNB*)malloc(sizeof(PHY_VARS_eNB));
  PHY_vars_eNb2 = (PHY_VARS_eNB*)malloc(sizeof(PHY_VARS_eNB));
...
int main(int argc, char **argv) {
  ...
  int i,l,aa,aarx;
  double sigma2, sigma2_dB=0,SNR,snr0=-16.0,snr1;
  u8 snr1set=0;
  //mod_sym_t **txdataF;
#ifdef IFFT_FPGA
  int **txdataF2;
  ...
  SCM_t channel_model=EVA;//Rayleigh1_anticorr;
...
//  number_of_cards = 1;
 ...
 //   }
  ...
  sprintf(tikz_fname, "Data/PBCH/bler_chan%d_nframes%d.csv",channel_model,n_frames);
   tikz_fd = fopen(tikz_fname,"w");
   fprintf(tikz_fd," SNR, Bler\n");
  
  s_re = (double **)malloc(2*sizeof(double*));
  s_im = (double **)malloc(2*sizeof(double*));
  s_re1 = (double **)malloc(2*sizeof(double*));
  s_im1 = (double **)malloc(2*sizeof(double*));
  s_re2 = (double **)malloc(2*sizeof(double*));
  s_im2 = (double **)malloc(2*sizeof(double*));
  r_re = (double **)malloc(2*sizeof(double*));
  r_im = (double **)malloc(2*sizeof(double*));
  r_re1 = (double **)malloc(2*sizeof(double*));
  r_im1 = (double **)malloc(2*sizeof(double*));
  r_re2 = (double **)malloc(2*sizeof(double*));
  r_im2 = (double **)malloc(2*sizeof(double*));
  ...
  for (i=0;i<2;i++) {
    ...
    s_re1[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    bzero(s_re1[i],FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    s_im1[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    bzero(s_im1[i],FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));    
    s_re2[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    bzero(s_re2[i],FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    s_im2[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    ...
    r_re1[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    bzero(r_re1[i],FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    r_im1[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    bzero(r_im1[i],FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    r_re2[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    bzero(r_re2[i],FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
    r_im2[i] = (double *)malloc(FRAME_LENGTH_COMPLEX_SAMPLES*sizeof(double));
  ...
  for (SNR=snr0;SNR<snr1;SNR+=1) {
	...
	printf("*********************** trial %d ***************************\n",trial);
      ...
      while (pbch_sinr>-2.0) {
		  ...
		  write_output("tx_pbch.txt","tx",&s_re[0][0],nsymb*OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES,1,1);
	  multipath_channel(eNB2UE,s_re,s_im,r_re,r_im,
			    2*nsymb*OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES,0);
	  	  write_output("rx_pbch.txt","rx",&r_re[0][0],nsymb*OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES,1,1);
	  if (interf1>-20) 
      ...
      sigma2_dB = 10*log10((double)tx_lev) +10*log10((double)PHY_vars_eNb->lte_frame_parms.ofdm_symbol_size/(12*NB_RB)) - SNR;
      if (n_frames==1)
	...
	if (n_frames==1) {
	  printf("rx_level data symbol %f\n",
		 10*log10((double)signal_energy(&PHY_vars_UE->lte_ue_common_vars.rxdata[0][3840],4*OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES)));
	}
	...
	*/
	  ...
	  if (l==((PHY_vars_eNb->lte_frame_parms.Ncp==0)?10:9)) {
	    ...
	    for (frame_mod4=0;frame_mod4<4;frame_mod4++) {
	      pbch_tx_ant = rx_pbch(&PHY_vars_UE->lte_ue_common_vars,
				    ...
				    SISO,
				    frame_mod4);
	      if ((pbch_tx_ant>0) && (pbch_tx_ant<4)) {//..wanlin Here antenna may be 4,so pbch_tx_ant should be <=4.But it do not influence the result here because of the antenna =1
    ...
    if (abstraction_flag==0) {
      printf("SNR %f : n_errors2 = %d/%d (BLER %e,40ms BLER %e,%d,%d), n_alamouti %d\n", SNR,n_errors2,ntrials*(trial),(double)n_errors2/(ntrials*(trial)),pow((double)n_errors2/(ntrials*(trial)),4),ntrials,trial,n_alamouti);
      if (write_output_file==1)
	fprintf(tikz_fd,"%f,%e\n",SNR,(double)n_errors2/(ntrials*(trial)));
    }
	if ((abstraction_flag==0) && ((double)n_errors2/(ntrials*(trial)))<1e-7)
	break;
  } // NSR
