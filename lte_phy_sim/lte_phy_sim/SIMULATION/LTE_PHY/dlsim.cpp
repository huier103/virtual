...
int main(int argc, char **argv) {
  ...
  double sigma2, sigma2_dB=10,SNR,snr0=10.0,snr1,rate,saving_bler=1;
  double snr_step=1,input_snr_step=1, snr_int=40;
  //change
  snr_int=1;
  //  
  ...
  unsigned char mcs,dual_stream_UE = 0,awgn_flag=1,round,dci_flag=0;
  ...
  */
  ...
  sprintf(tikz_fname, "Data/PDSCH/ber_bler_mcs%d_chan%d_nframes%d.csv",mcs,channel_model,n_frames);
   tikz_fd = fopen(tikz_fname,"w");
   fprintf(tikz_fd," SNR, Ber, Bler\n");
 for (i=0;i<2;i++) {
  ...
  if (input_fd==NULL) {
    ...
    */
	...
	trch_out_fd = fopen("dlsch_trch.txt","w");
	for (i=0;i<input_buffer_length;i++) {
	  input_buffer[k][i] = taus()&0xff;
	  for (int j=0;j<8;j++)
	    fprintf(trch_out_fd,"%d\n",(input_buffer[k][i]>>(7-j))&1);
	}
	fclose(trch_out_fd);
      }
  ...
  for (ch_realization=0;ch_realization<n_ch_rlz;ch_realization++){
		...
		*/		
		...
		amp = 1024;
	      ...
	      re_allocated = dlsch_modulation(PHY_vars_eNB->lte_eNB_common_vars.txdataF[eNB_id],
					      ...
					      num_pdcch_symbols,
					      PHY_vars_eNB->dlsch_eNB[k][0]);
		  write_output("txf.txt","txf",&PHY_vars_eNB->lte_eNB_common_vars.txdataF[eNB_id][0][0],512*14*10,1,1);
	...
	debug_msg("Wideband CQI tot %d dB, wideband cqi avg %d dB\n",
	  ...
	  */
	  ...
	  for(int iii=0;iii<PHY_vars_eNB->dlsch_eNB[0][0]->harq_processes[0]->TBS>>3;iii++)
	{
		for(int jjj=0;jjj<8;jjj++)
		{
			
			temp_tx=(input_buffer[0][iii]>>(7-jjj))&1;
			temp_rx=(PHY_vars_UE->dlsch_ue[0][0]->harq_processes[PHY_vars_UE->dlsch_ue[0][0]->current_harq_pid]->b[iii]>>(7-jjj))&1;
	...
	//printf("%s\n\n",stats_buffer);
      ...
      printf("\n**********************SNR = %f dB (tx_lev %f, sigma2_dB %f)**************************\n",
	     SNR,
	     (double)tx_lev_dB+10*log10((double)PHY_vars_UE->lte_frame_parms.ofdm_symbol_size/(NB_RB*12)),
	     sigma2_dB);
      ...
      fprintf(tikz_fd,"%f,%f,%f\n", SNR, (double)sum_bit_error/((double)PHY_vars_eNB->dlsch_eNB[0][0]->harq_processes[PHY_vars_UE->dlsch_ue[0][0]->current_harq_pid]->TBS*n_frames),(double)errs[0]/round_trials[0]);
	...
	if(blerr<1)
      ...
      if (((double)errs[0]/(round_trials[0]))<1e-7) 
	break;
    }// SNR
