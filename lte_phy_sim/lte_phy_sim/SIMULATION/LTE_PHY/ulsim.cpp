...
#include "PHY/defs.h"
#include "PHY/vars.h"
//#include "MAC_INTERFACE/vars.h"//..wanlin
#ifdef IFFT_FPGA
#include "PHY/LTE_REFSIG/mod_table.h"
#endif

//#include "ARCH/CBMIMO1/DEVICE_DRIVER/vars.h"//..wanlin
#include "SCHED/defs.h"
#include "SCHED/vars.h"
//#include "LAYER2/MAC/vars.h"//..wanlin
//#include "OCG_vars.h"//..wanlin
...
double snr0_awgn[30]={-14,-14,-13,-12,-12,-11,-10,-10,-9,-9,-7,-7,-7,-6,-6,-6,-5,-4,-4,-3,-2,-2,-3,-2,-1};
...
void lte_param_init(unsigned char N_tx, unsigned char N_rx,unsigned char transmission_mode,u8 extended_prefix_flag,u8 N_RB_DL,u8 frame_type,u8 tdd_config,u8 osf) {
	...
	// memset(PHY_vars_UE,0,sizeof(PHY_VARS_UE));//..wanlin
...
DCI0_5MHz_TDD_1_6          UL_alloc_pdu;
...
int main(int argc, char **argv) {
	...
	double sigma2, sigma2_dB=10,SNR,snr0=10.0,snr1,SNRmeas,rate;
	double snr_len=1.0;
	//int **txdataF, **txdata;
	int **txdata;
#ifdef IFFT_FPGA
	...
	unsigned int sum_bit_error;

	int eNB_id = 0;
	int UE_id = 0;
	unsigned char nb_rb=2,first_rb=0,mcs=16,round=0,bundling_flag=1;
	int n_frames=1000;
	unsigned char l;
	...
	//  FILE *rx_frame_file;
	double throughtput_without_hraq[1000];
	double throughtput[1000];
	...
	*/
	for(mcs=4;mcs<5;mcs++){
		...
		sprintf(tikz_fname, "Data/PUSCH/ber_bler_mcs%d_chan%d_nframes%d.csv",mcs,channel_model,n_frames);
		tikz_fd = fopen(tikz_fname,"w");
		fprintf(tikz_fd," SNR, Ber, error1,trials,Bler1,error2,trials,Bler2,error3,trials,Bler3,error4,trials,Bler4,Throughput without HARQ,Throughput\n");
		...
		int snr_t=0;
		for (SNR=snr0;SNR<snr1;SNR+=snr_len) {
			errs[0]=0;
			errs[1]=0;
			errs[2]=0;
			errs[3]=0;
			...
			round_trials[3] = 0;
			sum_bit_error=0;
			throughtput_without_hraq[snr_t]=0;
			throughtput[snr_t]=0;
			round=0;
			...
			for (trials = 0;trials<n_frames;trials++) {
				...
				fflush(stdout);
				round=0;
				while (round < 4) {//..wanlin
					printf("Mcs %d £ºSNR %2f : Trial %d : Round %d \n",mcs,SNR,trials,round);
					round_trials[round]++;
					if (round == 0) {
					...
					sigma2_dB =10*log10((double)tx_lev)  +10*log10((double)PHY_vars_UE->lte_frame_parms.ofdm_symbol_size/(PHY_vars_UE->lte_frame_parms.N_RB_DL*12)) - SNR;
					//tx_gain = sqrt(pow(10.0,.1*(N0+SNR))*nb_rb/((double)tx_lev*PHY_vars_eNB->lte_frame_parms.N_RB_UL));

					tx_gain = sqrt(pow(10.0,.1*(sigma2_dB+SNR))*nb_rb/((double)tx_lev*PHY_vars_eNB->lte_frame_parms.N_RB_UL));
					//tx_gain = 1.0;
					...
					// multipath channel
					// write_output("tx.txt","tx",&txdata[0][PHY_vars_eNB->lte_frame_parms.samples_per_tti*subframe],PHY_vars_eNB->lte_frame_parms.samples_per_tti,1,1);
					...
					//write_output("tx1.txt","tx1",&r_re[0][0],PHY_vars_eNB->lte_frame_parms.samples_per_tti,1,7);
					if (n_frames==1)
					...
					if (n_frames==1) {
						...
						printf("rx_level data symbol %f\n",10*log10((double)signal_energy((int*)&PHY_vars_eNB->lte_eNB_common_vars.rxdata[0][0][160+(PHY_vars_eNB->lte_frame_parms.samples_per_tti*subframe)],OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES/2)));
					}

					SNRmeas = 10*log10(((double)signal_energy((int*)&PHY_vars_eNB->lte_eNB_common_vars.rxdata[0][0][160+(PHY_vars_eNB->lte_frame_parms.samples_per_tti*subframe)],OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES/2))/((double)signal_energy((int*)&PHY_vars_eNB->lte_eNB_common_vars.rxdata[0][0][(PHY_vars_eNB->lte_frame_parms.samples_per_tti<<1) -PHY_vars_eNB->lte_frame_parms.ofdm_symbol_size],OFDM_SYMBOL_SIZE_COMPLEX_SAMPLES/2)) - 1)+10*log10((double)PHY_vars_eNB->lte_frame_parms.N_RB_UL/nb_rb);
					...
					//write_output("rx_bit.txt","rx",&PHY_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->b[0],PHY_vars_UE->ulsch_ue[0]->harq_processes[harq_pid]->TBS,1,9);
					if(round==0){
						u8 temp_tx,temp_rx;

					for(int iii=0;iii<PHY_vars_UE->ulsch_ue[0]->harq_processes[harq_pid]->TBS>>3;iii++)
					{
						for(int jjj=0;jjj<8;jjj++)
						{
							temp_tx=(input_buffer[iii]>>(7-jjj))&1;
							temp_rx=(PHY_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->b[iii]>>(7-jjj))&1;
							if(temp_tx!=temp_rx)sum_bit_error++;
						}
					}
					}
			...
			throughtput_without_hraq[snr_t]=(double)(round_trials[0]-errs[0])*(double)PHY_vars_UE->ulsch_ue[0]->harq_processes[harq_pid]->TBS/(double)(1.0*round_trials[0]*0.001);
			throughtput[snr_t]=(double)(round_trials[0]-errs[3])*(double)PHY_vars_UE->ulsch_ue[0]->harq_processes[harq_pid]->TBS/(double)(1.0*round_trials[0]*0.001);
			snr_t++;
			...
			printf("Errors %d/%d,Ber %e ,Bler %e\n",errs[0],round_trials[0],(double)sum_bit_error/((double)PHY_vars_UE->ulsch_ue[0]->harq_processes[harq_pid]->TBS*n_frames),(double)errs[0]/(round_trials[0]));
			fprintf(bler_fd,"Errors %d/%d,Ber %e ,Bler %e\n",errs[0],round_trials[0],(double)sum_bit_error/((double)PHY_vars_UE->ulsch_ue[0]->harq_processes[harq_pid]->TBS*n_frames),(double)errs[0]/(round_trials[0]));
			fprintf(tikz_fd,"%f,%f,%d,%d,%f,%d,%d,%f,%d,%d,%f,%d,%d,%f,%f,%f\n", SNR, 
				(double)sum_bit_error/((double)PHY_vars_UE->ulsch_ue[0]->harq_processes[harq_pid]->TBS*n_frames),
				errs[0],
				round_trials[0],
				...
				throughtput_without_hraq[snr_t-1]/(double)100000,
				throughtput[snr_t-1]/(double)100000);
