                  ...
                  loff_t *f_pos)
{
    ...
    if((flgrd+2)%2 != 0){
        ttmp = (unsigned char *)bbuf;
        printk("the second bbuf:\n");
        if(copy_to_user(buf, bbuf, count))
            printk("copy to user error\n");
        
        for(i = 0; i < 200; i++){   //count
            if(i%8 == 0) printk("n = %d ", i);
            printk("%02x ", ttmp[i]);
            if((i+1)%8 == 0) printk("\n");
        }
        memset(bbuf, 0, count);
...
try_read:
    if (!list_empty(&ppdma->rd_pending)) {
        ...
        //modify
        printk("\nread virt_addr: %x,phys_addr: %x\n", pentry->virt_addr, pentry->phys_addr);
        tmp = (unsigned char *)pentry->virt_addr;
        printk("virt data:\n");
        ...
        memcpy(bbuf, pentry->virt_addr, count);
        flgrd++;
        //modify end
